 ...
 */
function initSwup() {
  // The admin editor is a standalone full-page app. It loads main.js for the
  // shared header/footer, but SWUP must not run there, so every navigation
  // away from /admin is a full page load (the counterpart to ignoreVisit
  // skipping /admin from the site side).
  ...
  const swup = new Swup({
    ...
    // Ignore download links and other non-HTML resources. The admin editor is
    // a standalone full-page app with its own stylesheets; never transition
    // into it, or SWUP's HeadPlugin swaps the editor's <head> assets onto the
    // persisted page and they leak back onto the site on return. Navigation to
    // /admin is therefore always a full page load.
    ignoreVisit: (url) => {
